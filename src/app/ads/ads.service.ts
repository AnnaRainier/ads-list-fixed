...
export class AdsService {
 ...
 deleteAdItem(item): Observable<Ad[]> {
  ...
  createNewAd(item, ad?) {
    ...
    //let list = this.localStorageStore.getList('ads');
    if (ad && currentUserId) {
        if (ad.description && ad.title && currentUserId == ad.userId) {
          item.id = ad.id;
        item.title = ad.title;
        ...
        alert ('either data is not valid or you have no rights');
        //return of(list);
      }
    } else {
        if (item.description && item.title && currentUserId!= undefined) {
          if (list[list.length-1] !=undefined) {
            item.id = list[list.length-1].id + 1;
          } else item.id = 1;
        let user = this.localStorageStore.getList('userAuthorized');
